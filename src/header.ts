...
window.addEventListener("DOMContentLoaded", async (): Promise<void> => {
  ...
  ext.windows.onFocused.addListener(
    async (event: ext.windows.WindowEvent): Promise<void> => {
      if (extwindow?.id !== event.id) return;
      document.getElementsByClassName("window")[0].classList.add("focused");
    },
  );

  ext.windows.onUnfocused.addListener(
    async (event: ext.windows.WindowEvent): Promise<void> => {
      if (extwindow?.id !== event.id) return;
  ...
  ext.runtime.onMessage.addListener(async (_event, message) => {
    if (message.type === "title") {
      ...
      document.getElementsByClassName("window-title")[0].innerHTML =
        message.title;
...
_global.clickMaximize = async () => {
  try {
    ...
    if (isMaximized) {
      ...
      document
        .getElementsByClassName("window-action-restore")[0]
        .classList.add("window-action-maximize");
      document
        .getElementsByClassName("window-action-restore")[0]
        .classList.remove("window-action-restore");
      return;
    }
    if (extwindow === null) return;
    await ext.windows.maximize(extwindow.id);
    document
      .getElementsByClassName("window-action-maximize")[0]
      .classList.add("window-action-restore");
    document
      .getElementsByClassName("window-action-maximize")[0]
      .classList.remove("window-action-maximize");
  } catch (error) {
    console.error(error);
  }
};
